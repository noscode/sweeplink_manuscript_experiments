...
def process_and_print_intermediate_results(char_name, char_val, ind):
    char_config = config.get_char_config(char_name)
    sel_list = [0.0] + config.get_sel_list(char_name)
    tool_name = config.get_tool_name(char_name, char_val)

    # selection
    data = results.get_data_per_s_for_ind(char_name, char_val, ind, s_trace_file=True)
    metrics_data = metrics.calculate_metrics(
        parsed_data=data,
        sel_list=sel_list,
        threshold=config.THRESHOLDS[tool_name],
    ...
    # Pop size
    posterior = results.get_trace_data_for_ind(char_name, char_val, ind)['N_pop0']
